...
function selectAnswer () {
  ...
  if(singleChoice){
    $('.card-select').each(function(i,e){
      $("i",e).removeClass('glyphicon-check').addClass('glyphicon-unchecked');		
      var parent = $(e).parent('.card');
      parent.removeClass('right wrong');
      var o = parent.data().o;
  ...
  if (userQuestions[i].value !== "") {
   ...
   for (var j = 0; j < userAnswers.length; j+=2) {
    if (userAnswers[j].value !== "") {
     ...
     if(userAnswers[j+1].value == "true") correctAnswers++;
    }
   }
   if(correctAnswers > 1) singleChoice = false;
...
function createAnswer (o) { 
 ...
 if (o.answer.hint !== "") {
  $card.append(
   ...
   ' data-content="'+AppClient.linkifyText(o.answer.hintTTS == '2' ? '' : o.answer.hint).replace(/"/g,'&quot;')+'">'+
   '<span class="glyphicon glyphicon-info-sign infoicon"></span></span>');
 }
 
 if (o.answer.hintTTS == '1' || o.answer.hintTTS == '2')
...
function quitApp () {
 ...
 if (isEvalMode) {
  ...
  showProgress();
  appEnded = true;   
   $('.card-select').css({'pointer-events':'none'});
 }else{
  AppClient.setSolved();   
 }
...
///////////////////////////////////////////////////////////////////////////////
// check if all elements are answered and inform upper frames via postmessage
///////////////////////////////////////////////////////////////////////////////
function checkAppCompleted () {
 // this app is completed when all questions answered and quitApp() is called
...
/////////////////////////////////////////////////////////////////////////////
// hold the checkedResults Array for AppClient.setChecked globally
/////////////////////////////////////////////////////////////////////////////
var checkedResults = [];
function checkSolution () {
 ...
 // prepare AppClient.setChecked
  checkedResults.push(questionCorrectAnswered ? 1 : 0);
 ...
 if (questionCorrectAnswered) {
  ...
  if (currentQuestion + 1 >= questions.length) 
   ...
   currentQuestion++;
   changeSolutionButton();
   //setTimeout(function () {changeSolutionButton();nextQuestion();},1500);
  } 
