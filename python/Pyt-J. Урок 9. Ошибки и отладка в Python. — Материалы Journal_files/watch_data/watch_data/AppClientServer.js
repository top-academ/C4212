...
addAppClientTranslation("RU",
                        "APPCLIENT_VIDEODELTED","Данное видео не существует или ещё не опубликовано.");
addAppClientTranslation("RU",
...
//          AppClientConnection.socket.setAppSolved(self.showID,value,self.startTime);
  ...
  this.setChecked = function() {
    ...
    //if (AppClientConnection.socket)
    //    AppClientConnection.socket.setAppChecked(self.showID,v,self.startTime);

    self.postMessage("AppChecked|"+self.showID+"|"+t+'|'+v);
    if(self.Tool == 1){
      self.postMessage("AppCollection|"+self.showID+"|"+t+'|'+v);
    }
  }
...
function stopAllYouTubePlayers(index) {
  for (var i = 0; i < YouTubePlayers.length; i++) {
    var c = YouTubePlayers[i];
    if(i == index) continue;
    if(c && (!c.onStateChangeSet || c.firstStart)) continue; // not loaded yet
    if(c) c.pause();
  }
}
//////////////////////////////////////////////////////////////////////////////
...
function onYouTubePlayerStateChange(evt) {
    ...
    if(t.video) {
      if (t.playerState === YT.PlayerState.BUFFERING || t.playerState === -1) {
          // Player is buffering or unstarted, possible ad is loading
          t.video.handleAdCheck();
      } else if (t.playerState === YT.PlayerState.PLAYING) {
          // Main content is playing
          t.isAdPlaying = false;
          t.video.hideAdOverlay();
      } else if (t.playerState === YT.PlayerState.ENDED && t.video.isAdPlaying) {
          // Player ended while an ad was playing, prepare for next possible ad
          t.video.handleAdCheck();
      } else {
          t.video.hideAdOverlay(); // Hide overlay for other states
      }
    }
    
    if ((t.playerState == -1 || t.playerState == 5 || t.playerState == 2) && t.firstStart) {
        t.firstStart = false;
    }
    if (t.playerState == 1) {
...
function YouTubeVideo(YouTubeVideoID, isAudio) {
  ...
  this.handleAdCheck = function () {
     var self = this;
    // If ad already detected, don't reset timer
    if (this.isAdPlaying) return;

    this.overlayTimeout = setTimeout(function () {
      if (self.playerState === YT.PlayerState.BUFFERING || 
          self.playerState === -1) {
        // Still in a state that indicates possible ad
        self.isAdPlaying = true;
        self.showAdOverlay();
        // check in 500ms if ad is playing, if not - hide the panel
        this.overlayTimeout = setTimeout(function(){
          if(self.isAdPlaying && !self.videoCurrentTime){
  ...
  this.showAdOverlay = function() {
     var self = this;
     this.mute()
     var d = document.getElementById('adOverlay'+this.ID)
     d.parentElement.style.overflow = "visible"
     d.style.display = 'block';
     d.onclick = function(){
       self.hideAdOverlay();
     };
   ...
   this.hideAdOverlay = function () {
     clearTimeout(this.overlayTimeout); // Cancel any pending overlay show
     this.unmute()
     var d = document.getElementById('adOverlay'+this.ID)
     d.parentElement.style.overflow = "hidden"
  ...
  this.updatePlayerInfo = function () {
    ...
    var adTimer = document.getElementById('adCountdownTimer'+this.ID);
    if(adTimer) adTimer.innerText = Math.round(this.videoCurrentTime);
...
function YouTubeAudio(YouTubeVideoID) {
  ...
  this.loadPlayer = function (div) {
    ...
    if (!this.contentFilled) {
      this.contentFilled = true;
      insertIn.innerHTML = '<table style="border: 1px solid gray; background:white;width:100%; height:30px" cellspacing="0" cellpadding="0"><tr style="position:relative"><td><div id="videoControls' + this.ID + '" style="background: white; font-size: 0.75em; "><table style="width:100%" cellspacing="0" cellpadding="3"><tr><td width="30" align="center">'+(!this.video.isFromURL ? '<div id="videoPlaceholderOuter' + this.ID + '" style="position:absolute;height:25px; opacity:0.01;width:35px;overflow:hidden;padding:0"><div style="position:absolute;left:-82px;top:-87px;height:200px; width:200px;" id="videoPlaceholder' + this.ID + '"></div></div>' : '')+'<button type="button" style="position:static;display:inline;font-size:15px;cursor:pointer; font-weight:bold;text-decoration:none; height:25px; width:25px; padding:0px; text-align:center; background:#F3F3F3; color:#333;" class="btn btn-primary" id="playpauseButton' + this.ID + '" onclick="YouTubePlayers[' + (this.index) + '].playpause();"> \u25B6 </button> </td><td  style="overflow: hidden;position:relative;"><div id="adOverlay'+ this.ID +'" style="display:none;position:absolute;left:-30px;top:0;width:calc(100% + 90px);height:100%;background: #156082;color:white;font-size:1rem;z-index:1;text-align:center;padding:4px;">YouTube Ad - please wait</div>' + this.video.createTrackBarItems() + '</td><td width="60" align="right"><span style="padding:1px;padding-left:3px;white-space:nowrap" id="videoTime' + this.ID + '" > </span></td></tr></table></div></td></tr></table>';
      if(!window.YouTubePlayersLoading)window.YouTubePlayersLoading = [];
      window.YouTubePlayersLoading.push({div:div, index:this.index});
      if(window.YouTubePlayersLoadingTimeout) 
        clearTimeout(YouTubePlayersLoadingTimeout);
      YouTubePlayersLoadingTimeout = setTimeout(function(){
        // now load all from window.YouTubePlayersLoading
        var loadDivNow = function(index,div,timeout){
          setTimeout(function(){
            YouTubePlayers[index].loadPlayer(div);
          },timeout);
        };
        
        for(var i = 0; i < window.YouTubePlayersLoading.length; i++){
          loadDivNow(window.YouTubePlayersLoading[i].index,window.YouTubePlayersLoading[i].div,i*250);
        }
      },200);
